...
TET_API_CREDS="api_credentials_osanniko.json"
...
#Collecting IDs for all Tetration Apps

#for app in resp.json():
#    appIDs.append(app['id'])
#    appNames.append(app['name'])

app_key = "59c6171e755f025d2d52301a" #Meat App
appIDs = [app_key]

#Collecting Policies for selected Tetration Apps
...
#Creating blank Access Control Policy

fmc_acp = AccessPolicy('Tetration ACP','BLOCK')
fmc_acp = fmc.create(fmc_acp)
print('Created Access Control Policy ' + fmc_acp.name)

#Creating Host Objects and Network Group Objects based on Tetration clusters for selected App
...
for policy in policies:
    ...
    if policy.consumerFilterID in clusters.keys(): #Is source network a cluster?
        ...
        nets = []
        if policy.consumerFilterName != 'Default':
            rule.sourceNetworks = {'objects': [fmc_networkgroups[policy.consumerFilterID]]}
        else:
    ...
    if policy.providerFilterID in clusters.keys(): #Is destination network is a cluster
        ...
        nets = []
        if policy.providerFilterName != 'Default':
            rule.destinationNetworks = {'objects': [fmc_networkgroups[policy.providerFilterID]]}
        else:
        ...
        if policy.consumerFilterName != policy.providerFilterName:
            if (l4param['proto'] == 6) or (l4param['proto'] == 17):
                if l4param['port_min'] == l4param['port_max']: #If protocol is TCP or UDP
                    rule.destinationPorts['literals'].append({'port': str(l4param['port_min']), 'protocol': str(l4param['proto']), 'type': 'PortLiteral'})
                else:
                    rule.destinationPorts['literals'].append({"port": str(l4param['port_min'])+'-'+str(l4param['port_max']), 'protocol': str(l4param['proto']), 'type': 'PortLiteral'})
...
#Push rules to FMC        
#fmc_acp_rules = fmc.create(fmc_rules[5])
